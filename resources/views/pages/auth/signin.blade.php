<x-layouts title="Sign In" class="my-auto">
    <form action="{{ route('auth.signin') }}" method="POST" class="flex flex-col mx-auto my-10">
        ...
        <div class="flex items-center gap-2 mx-auto bg-gray-50 border p-5 rounded-xl w-full justify-center">
            <h1 class="text-xl font-bold text-gray-400">Masuk ke</h1>
            <x-brand />
        </div>
        <p class="text-center max-w-xs mt-6">Pinjam Buku Jadi Lebih Mudah dan Teratur dengan Sistem Digital!</p>

        @if (session('success'))
        <div class="bg-green-500 text-white p-4 mt-5 rounded mb-4 w-full">
            {{ session('success') }}
        </div>
        @endif
        ...
        <div class="flex flex-col mt-10 gap-4">
            ...
            <div class="space-y-2 w-full text-center">
                <button class="btn btn-md btn-error w-full text-white">Masuk</button>
                <p class="text-sm">Belum punya akun? <a href="/auth/signup" class="text-red-500 underline">Daftar</a></p>
            </div>
        </div>
    </form>
</x-layouts>
